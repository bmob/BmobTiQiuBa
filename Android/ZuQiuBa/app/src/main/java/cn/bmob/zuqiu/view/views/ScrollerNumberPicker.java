...
public class ScrollerNumberPicker extends View {
    ...
    private void actionUp(int move) {
        ...
        if (move > 0) {
            ...
            for (int i = itemList.size() - 1; i >= 0; i--) {
                if (itemList.get(i).isSelected()) {
                    newMove = (int) itemList.get(i).moveToSelected();
                    if (onSelectListener != null && itemList.size() !=0)
                        onSelectListener.endSelect(itemList.get(i).id,
                                itemList.get(i).itemText);
                    break;
                }
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray attribute = context.obtainStyledAttributes(attrs,
                R.styleable.CityNumberPicker);
        unitHeight = (int) attribute.getDimension(
                R.styleable.CityNumberPicker_unitHight, 32);
        normalFont = attribute.getDimension(
                R.styleable.CityNumberPicker_normalTextSize, 14.0f);
        selectedFont = attribute.getDimension(
                R.styleable.CityNumberPicker_selecredTextSize, 22.0f);
        itemNumber = attribute.getInt(R.styleable.CityNumberPicker_itemNumber, 7);
        normalColor = attribute.getColor(
                R.styleable.CityNumberPicker_normalTextColor, 0xff000000);
        selectedColor = attribute.getColor(
                R.styleable.CityNumberPicker_selecredTextColor, 0xffff0000);
        lineColor = attribute.getColor(R.styleable.CityNumberPicker_lineColor,
                0xff000000);
        maskHight = attribute.getDimension(R.styleable.CityNumberPicker_maskHight,
                48.0f);
        noEmpty = attribute.getBoolean(R.styleable.CityNumberPicker_noEmpty, false);
        isEnable = attribute
                .getBoolean(R.styleable.CityNumberPicker_isEnable, true);
        attribute.recycle();
